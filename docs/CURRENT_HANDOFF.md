...
- 当前分支：`main`
- 当前业务基线提交：`a13d623 feat: 内置本地响应式万年历` 为 alpha75 起点；alpha77 图标业务提交及交接文档以现场 `HEAD` 为准
- 版本：`0.3.0-alpha77`，`versionCode 78`
- 交接前业务工作区：干净
...
- 自适应图标继续使用该前景与与素材边缘一致的白色背景，不重绘、改色、裁切主体或添加外部黑白托盘。
- API 35 的系统应用信息页和 Pixel Launcher 应用抽屉已实测显示；OPPO/ColorOS 的实际蒙版仍须真机授权后单独验收。
...
4. 若没有新的样本、真机或发布授权，保持 alpha77，不为了制造进度擅自增加功能或升级 alpha；八图超时仅在再次发生时作为复现诊断任务。
