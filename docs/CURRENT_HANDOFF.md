...
- 完整备份逐例计划、全屏工作台、明文/密码恢复、非空库导入、带附件范围合并和附件事务
  已有实现；本增量补齐真实模拟器进程强杀与冷启动恢复证据。
- v1.0 总方案仍未完成；单命例可选附件包及后续算法、问真截图迁移仍待落地。
...
- 合并日志同时记录目标提交前和提交后载荷哈希；进程若在文件移动后、Room 更新前退出，
  启动恢复可确认目标仍是旧载荷并安全回收孤儿附件，不会把既有目标误当成部分提交；
- 新增两阶段 instrumentation：第一阶段在 App 真实私有目录写入日志、暂存与最终孤儿文件
  并保持进程；主机确认 PID 和边界状态后执行真实 `force-stop`；
- 第二阶段读取种子 PID 并断言当前 PID 不同，冷启动 `MainActivity` 后等待生产恢复链清除
  日志、暂存和最终孤儿目录；
- 可重复脚本 `scripts/run_restore_process_recovery_e2e.sh` 本次以 PID 2817 通过；脚本
  硬性拒绝非 `emulator-*` 序列号，不会触碰 OPPO；
- Android UI 已开放最终确认按钮，成功后清理旧预览/计划并显示导入、保留、合并、跳过
  和附件数量摘要；
- 完整恢复预览改为全屏 `LazyColumn` 工作台：顶部持续显示已决策/总数，备份概览、逐例
  冲突卡片和底部计划/提交动作分层，案例数量增加时不再一次组合全部内容；
- “明确将全部命例设为跳过”是独立显式用户动作，原子生成每例 `SKIP` 决策并使旧计划
  失效；不会跳过最终方案检查或直接执行恢复；
- 系统返回和顶部关闭均回到命例列表；进入范围合并时临时隐藏工作台，取消后恢复同一批
  决策；
- 共享合并分析现在统计实际新增记录/历史/事件引用的来源附件；工作台明确提示附件会按
  实际选择复制并跨模块去重；
- 执行范围合并时只为实际新增内容所需附件生成新 ID 与事务路径，同时重写记录、历史和
  事件引用；未选来源附件不会导入；
- 目标既有附件、字段证据和文件保持不变；同一来源附件被记录与事件共同引用时只复制一份；
- 强制数据库 `UPDATE` 失败的集成测试证明：目标聚合回滚、本地附件字节不变、本事务新
  文件清理；移除故障后同一计划可成功恢复真实来源附件；
- 错误弹窗保留协议错误代码，导出成功明确图片仅保留引用；
- 启用 App `BuildConfig.VERSION_NAME`，导出文件记录真实当前版本；
- Debug 版本升级到 `0.3.0-alpha21`。
...
- 本地自动化：90 条唯一单元契约；Debug/Release 变体合计 167 次执行，0 失败；
...
- API 35 模拟器 `ExpenseCapture_API35`：
  ...
  - 同一流程还完成重复提示与确认→编辑→新增带标题/事业类别事件→详情及历史读回
    ...
    →普通 ZIP 在全屏惰性工作台滚动到真实活动冲突候选→进入范围合并→空范围确认禁用
    →返回同一工作台→明确批量设为跳过→生成零写入恢复计划→最终确认→重新打开同一文件
...
`app/build/outputs/apk/debug/NanfengBazi-Android-v0.3.0-alpha21-debug.apk`

大小：9,906,658 bytes

SHA-256：`cb6d74a6db424eaf9102c7e77e82c4506b6b17657b6bc7695c641af09c14a311`
...
- 没有 OPPO Find N5、系统文件含附件提交或真实问真样本证据。

## 下一安全增量

继续 Stage 3B：

1. 实现单命例可选附件包及其明文/密码、预览、冲突和附件事务；
2. 完成 Stage 3B 剩余数据安全边界；
3. 进入 Stage 4A 的出生输入与算法结果完整化。
