...
- 最新细化：基本排盘与万年历继续共用 `BasicShenShaRules`，不再由入口自行漏传神煞；专业九列藏干使用完整十神，五层时间轴展示全部地支藏干及十神简称。流日改为当前整月固定横向序列，流时（含 23:00）回填同一民用日期，避免点击后卡片偏移或无故改月；五层卡片提高明度，隐藏小字预留行高，基本信息姓名／性别使用独立不裁切行。
