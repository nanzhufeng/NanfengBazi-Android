...
- 本增量完成完整备份逐命例冲突预演，并保持明文/密码系统文件入口严格零写入。
...
`app/build/outputs/apk/debug/NanfengBazi-Android-v0.3.0-alpha13-debug.apk`

大小：10,094,990 bytes

SHA-256：`da95a34cf3f86d6b3f1ee9d21f72f0a03c94353f2138321f54f955167107847b`
...
- 冲突预演尚无逐例决策编辑器；密码完整备份恢复、附件事务、非空库两阶段提交和进程
  强杀恢复仍未实现；
...
1. 为完整备份建立逐命例跳过/保留两份/范围合并决策和预演过期令牌；
2. 补齐附件二进制导入、引用计数、原子提交和失败回滚；
3. 接通明文/密码备份两阶段提交，并补进程重启恢复日志。
