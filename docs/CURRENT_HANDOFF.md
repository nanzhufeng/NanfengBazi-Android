...
- 基本信息使用独立居中身份区、连续交错信息表和可信命盘摘要；其余三页使用紧凑身份条。基本排盘为年月日时四列密集矩阵，包含日期、主星、天干、地支、藏干／十神、副星、星运、自坐、空亡和纳音。
- 专业细盘首屏按“流时—流日—流月—流年—大运—原局四柱”九列展示，再呈现岁运定位、八步大运和当前十年流年密集表。断事笔记使用“命主反馈／师傅点评”切换；职业、学历、财富、婚姻、健康回归反馈页，与正文及事件时间线直接阅读。
...
- API 35 `emulator-5554`：`CaseDetailReferenceLayoutTest` 1/1 通过，覆盖共享头部、基本信息、藏干排盘、岁运轴和断事时间线；`RecordPreviewFixtureTest` 仅在显式 `seedPreviewCases=true` 时生成合成视觉数据并 1/1 通过。
- 用户提供的五张问真参考与模拟器四页实际截图已并排检查；修正了选中指示和操作链接的金色层级。
