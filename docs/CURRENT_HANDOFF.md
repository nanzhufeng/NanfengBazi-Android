...
- 完整备份逐例计划、全屏工作台、明文/密码恢复、非空库导入和附件事务已有实现；本增量
  补齐记录/事件范围合并时的来源附件复制。
- v1.0 总方案仍未完成；设备进程强杀恢复证据、单命例附件包取舍及后续算法、问真截图
  迁移仍待落地。
...
`app/build/outputs/apk/debug/NanfengBazi-Android-v0.3.0-alpha20-debug.apk`

大小：10,118,540 bytes

SHA-256：`a8569bc1f0a6fe2db5c8e421264387291e16a84db31a2413ddf8bf0f7e433665`
...
1. 在干净模拟器执行真实进程强杀恢复测试；
2. 评估单命例附件包与永久清理引用计数，完成 Stage 3B 剩余数据安全边界；
