...
- 完整备份逐例计划、明文/密码恢复、非空库导入、范围合并和真实附件事务已有实现；本增量
  将批量决策从长弹窗改为全屏惰性列表工作台。
- v1.0 总方案仍未完成；带附件模块合并、设备进程强杀恢复证据及后续算法、问真截图迁移
  仍待落地。
...
`app/build/outputs/apk/debug/NanfengBazi-Android-v0.3.0-alpha19-debug.apk`

大小：9,897,586 bytes

SHA-256：`cea5dfe67edee61a62cb849e09017bac98be799582942fdbbe948461f128ac64`
...
1. 为带附件模块合并设计“随模块复制附件”或显式排除来源证据的可解释语义和测试；
2. 在干净模拟器执行真实进程强杀恢复测试；
3. 评估单命例附件包与永久清理引用计数，完成 Stage 3B 剩余数据安全边界。
