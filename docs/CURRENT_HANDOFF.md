...
- API 35 `emulator-5554`：`CaseDetailReferenceLayoutTest` 1/1 通过，覆盖共享头部、基本信息、藏干排盘、五层岁运点击联动、日期详情、合冲神煞和断事时间线；`TymeProfessionalFortuneResolverTest` 覆盖九列十神、五层候选、交运首年定位与规则版本。`RecordPreviewFixtureTest` 仅在显式 `seedPreviewCases=true` 时生成合成视觉数据，不冒充真实样本。
