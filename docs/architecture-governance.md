...
| 启动图标 | `design/assets/app-icon-master.png` → `scripts/build_android_launcher_icon.py` → `app_icon_source`、`mipmap-*` / adaptive icon | 用户提供 PNG 是唯一当前母版；两层 adaptive 构建与启动器核对严格按 [启动图标保真构建规范](app-icon-fidelity-standard.md) 执行；上一版 JPG 只归档、不参与构建 |
...
| 管理动作 | 详情右上角菜单 → 既有用例 | 仅保留编辑基本资料、分组与标签、保存命盘长图到图库、分享命盘长图和软删除／恢复；客观摘要与外部分析能力继续保留内部合同，但不再作为详情菜单入口 |
| 国内行政区目录 | `china-districts.csv` → `BirthplaceCatalog` | 锁定上游提交生成 2,846 条省／市／区县名称；仅补全选择目录和北京民用时，不用区县名伪造经纬度或真太阳时证据 |

## alpha82 记录编辑、列表手势与图片交付边界

| 概念 | 唯一所有者/入口 | 边界 |
|---|---|---|
| 命例显示名称 | `CaseFormState.name` 与兼容字段 `alias` 同步写入 → `CaseSummary` | 当前界面只提供一个“姓名”入口；旧数据仍可从 `alias` 回退显示，但不得再次出现两个可编辑名称或重复详情行 |
