...
| 页面消费 | `StageTwoViewModel` → 专业细盘 | 观察日期、时分进入 `SavedStateHandle`；九列四柱矩阵与大运/流年/流月/流日/流时五层时间轴共同消费 `ProfessionalFortuneResolver` 的十神、逐条藏干和生日整年周岁。九列使用完整十神，时间轴地支逐条展示全部藏干及十神简称；流日为整月横向候选、流时始终回填同一民用日期。UI 只排版，不计算年龄、藏干或月界；正文不显示节气区间或复制诊断 |
