...
@Serializable
internal data class SingleCaseBundleEncryptionHeader(
...
internal class SingleCaseBundleEncryption(
    ...
    fun encryptingStream(
        ...
        val header = SingleCaseBundleEncryptionHeader(
        ...
        val headerBytes = DomainJson.encodeToString(
            SingleCaseBundleEncryptionHeader.serializer(),
            header,
        ).encodeToByteArray()
    ...
    fun decryptAfterMagic(
        ...
        val header = DomainJson.decodeFromString<SingleCaseBundleEncryptionHeader>(
        ...
        CipherInputStream(data, cipher).use { decrypted ->
            Files.newOutputStream(targetZip).buffered().use { output ->
                ...
                while (true) {
                    ...
                    if (count < 0) break
                    total += count
                    require(total <= MAX_DECRYPTED_BUNDLE_BYTES) {
                        "Decrypted single-case bundle is too large"
                    }
    ...
    private fun validateHeader(header: SingleCaseBundleEncryptionHeader) {
    ...
    companion object {
        const val CONTAINER_TYPE = "nanfeng-bazi-single-case-bundle-encrypted"
        const val PROTECTION_VERSION = 1
        const val MAX_DECRYPTED_BUNDLE_BYTES = 1024L * 1024 * 1024
        const val MAX_HEADER_BYTES = 16 * 1024
        val MAGIC_BYTES = "NANFENG_BAZI_SINGLE_CASE_BUNDLE_ENCRYPTED\n".encodeToByteArray()
    }
}
