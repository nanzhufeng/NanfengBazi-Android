...
internal const val AutoGeneratedCaseAliasPrefix = "某某"

/** 只在用户未填写姓名而主动保存时使用；从最小未占用序号开始，且不复用回收站中的名称。 */
internal fun nextAutoGeneratedCaseAlias(existingAliases: Collection<String>): String {
    val occupied = existingAliases.mapTo(mutableSetOf()) { it.trim() }
    var sequence = 1
    while ("$AutoGeneratedCaseAliasPrefix$sequence" in occupied) {
        sequence += 1
    }
    return "$AutoGeneratedCaseAliasPrefix$sequence"
}
