...
internal class FakeCaseRepository : CaseRepository {
    ...
    val findByIdsRequests = mutableListOf<Set<String>>()
    val listGroupRequests = mutableListOf<CaseLibraryType?>()
    ...
    var beforeFindByIds: suspend (Set<String>) -> Unit = {}
    ...
    override suspend fun findByIds(ids: Set<String>): Map<String, BaziCase> {
        beforeFindByIds(ids)
        findByIdsRequests += ids
        readFailure?.let { throw it }
    ...
    override suspend fun moveBlankPlaceholderCasesToTrash(deletedAt: Instant): Int {
        val targetIds = stored.values
            .filter { case ->
                case.libraryType == CaseLibraryType.USER &&
                    case.deletedAt == null &&
                    (case.alias.isBlankPlaceholderName() || case.name.value.isBlankPlaceholderName()) &&
                    case.textRecords.isEmpty() &&
                    case.events.isEmpty()
            }
            .map { it.id }
            .toSet()
        if (targetIds.isEmpty()) return 0
        return moveCasesToTrash(targetIds, deletedAt)
    }
    ...
    override suspend fun listGroups(libraryType: CaseLibraryType?): List<CaseGroup> {
        listGroupRequests += libraryType
        return groupCatalog.filter { libraryType == null || it.libraryType == libraryType }
    }
...
private fun String?.isBlankPlaceholderName(): Boolean {
    val value = this ?: return false
    if (value == "某某") return true
    val suffix = value.removePrefix(AutoGeneratedCaseAliasPrefix)
    return suffix.length < value.length && suffix.isNotEmpty() && suffix.all { it in '0'..'9' }
}
